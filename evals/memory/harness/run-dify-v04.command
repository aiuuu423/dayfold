...
LOG="$REPO_ROOT/local/verification/dify-v04-run.log"
...
finish() {
  ...
  echo
  if [[ $status -eq 0 ]]; then
    echo "Dify Workflow v0.4 baseline 已完成。"
  else
    echo "Dify Workflow v0.4 baseline 未完成，退出码：$status"
  fi
  read -r -p "按回车关闭窗口..." _
  exit $status
}
...
echo "Dayfold P1 Dify Workflow Memory Extraction v0.4 baseline"
...
export DAYFOLD_EVAL_BASELINE="siliconflow-v04-results.json"
export DAYFOLD_EVAL_RESULT="dify-v04-results.json"
export DAYFOLD_EVAL_REPORT_DIR="dayfold-p1-dify-v04-report"
export DAYFOLD_EVAL_REPORT_TITLE="Dayfold P1 Dify Workflow v0.4 Baseline"
export DAYFOLD_EVAL_VERSION="dify-v0.4"
export DAYFOLD_EVAL_BASELINE_LABEL="SiliconFlow Direct v0.4"
